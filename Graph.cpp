#include <iostream>#include "graph.h"#include <string>#include <map>#include <set>int Graph::n(string city) {
...
vector<Flight*> Graph::cheapestPath(string &origin, string &dest) {        //want to use djikstra's or another algorithm to get shortest path//returns a vector of flight numbers}
...
void Graph::printBoardingPass(string &filter, Flight* thisFlight) {
    ...
    else {      //flight exists and a valid filter was passed in        // print out all relevant graph info by accessing the graph from map        cout << "Based on " << filter << " flight filter, the best flight(s) for you is:" << endl;
