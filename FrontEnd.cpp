...
string WelcomeScreen::showWelcomeScreen() {
    ...
    // Setup welcome text
    sf::Text welcomeText("Welcome to DSA Project 3! \nBy Eneida Escobar, Marielle Doenges, \nand Maggie Snead", font, 30);
    welcomeText.setFillColor(sf::Color::Black);
    welcomeText.setPosition(50, 20);
...
void MapScreen::moveTextLocation(string interaction){
    ...
    }else if(interaction == "write output"){
        setTextRight(output, 1450, 600);
    }
}
...
void MapScreen::putPromptsToScreen(sf::RenderWindow &window, string interactionStatus) {
    ...
    if(interactionStatus == "0"){
        ...
        window.draw(prev);
        window.draw(yes);
    }else if(interactionStatus == "write output"){
        window.draw(output);
    }
}
            ...
            if(event.type == sf::Event::MouseButtonReleased){
                ...
                if(zoomIn.getGlobalBounds().contains(window.mapPixelToCoords(mouse))){
                    ...
                    if(interaction == "confirm1"){
                        ...
                        output.setString(final[0]);
                        interaction = "write output";
                    }else if(interaction == "create budget"){
...
Ticket::Ticket(vector<std::string> yourInformation) {
    ...
    if(yourInformation[0] == "NA"){
        ...
        originCountry.setPosition(219, 290);
...
void Ticket::drawRepeatedItems(sf::RenderWindow& window) {
    name.setPosition(215, 200);
    window.draw(name);
    name.setCharacterSize(17);
    name.setPosition(861, 185);
    window.draw(name);

    originCity.setPosition(210, 265);
    window.draw(originCity);
    originCity.setPosition(939, 200);
    window.draw(originCity);

    destCity.setPosition(660, 265);
    window.draw(destCity);
    destCity.setPosition(907, 230);
    window.draw(destCity);

    depTime.setPosition(309, 345);
    window.draw(depTime);
    depTime.setPosition(946, 339);
    window.draw(depTime);
}

void Ticket::draw() {
    // Initialize the main window of the application with a specific size and title.
    sf::RenderWindow window(sf::VideoMode(1200, 600), "Flight Ticket Booking", sf::Style::Close);
    sf::Sprite background;
    sf::Texture backT;
    backT.loadFromFile("images/boardingPass.jpg");
