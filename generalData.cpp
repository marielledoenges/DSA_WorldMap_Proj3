...
string WorldInfo::makeFlightNum(set<int> &taken) {
    int flightNum = rand() % 10000;
    while(taken.find(flightNum) != taken.end()){
        flightNum = rand() % 10000;
    }
    taken.insert(flightNum);
...
bool WorldInfo::writeFile(string filename){
    ...
    string cityFrom, cityTo, countryFrom, countryTo, price, distance, dur, TZdiff, month, depTime, international, date, flightNum;
    int min, max;
    for(int i = 0; i < 7200; i++){
        ...
        flightNum = makeFlightNum(allFlightNums);
